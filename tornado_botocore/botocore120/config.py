...
def multi_file_load_config(*filenames):
    ...
    for filename in filenames:
        ...
        except botocore120.exceptions.ConfigNotFound:
            continue
...
def raw_config_parse(config_filename):
    ...
    if path is not None:
        ...
        if not os.path.isfile(path):
            raise botocore120.exceptions.ConfigNotFound(path=path)
        cp = configparser.RawConfigParser()
        try:
            cp.read(path)
        except configparser.Error:
            raise botocore120.exceptions.ConfigParseError(path=path)
        else:
                ...
                for option in cp.options(section):
                    ...
                    if config_value.startswith('\n'):
                        ...
                        try:
                            ...
                            raise botocore120.exceptions.ConfigParseError(
                                path=path)
