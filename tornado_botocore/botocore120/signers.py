...
import botocore120
import botocore120.auth
from botocore120.awsrequest import create_request_object, prepare_request_dict
from botocore120.exceptions import UnknownSignatureVersionError
from botocore120.exceptions import UnknownClientMethodError
from botocore120.exceptions import UnsupportedSignatureVersionError
from botocore120.utils import fix_s3_host
...
class RequestSigner(object):
    ...
    * before-sign: Allows mutating the request before signing.
    ...
    :type credentials: :py:class:`~botocore120.credentials.Credentials`
    :param credentials: User credentials with which to sign requests.
    :type event_emitter: :py:class:`~botocore120.hooks.BaseEventHooks`
    ...
    def sign(self, operation_name, request):
        ...
        if signature_version != botocore120.UNSIGNED:
    ...
    def get_auth(self, signing_name, region_name, signature_version=None,
        ...
        :type signature_version: string
        :param signature_version: Signature name like ``v4``.
        :rtype: :py:class:`~botocore120.auth.BaseSigner`
        ...
        cls = botocore120.auth.AUTH_TYPE_MAPS.get(signature_version)
        if cls is None:
            ...
            if cls.REQUIRES_REGION:
                if self._region_name is None:
                    raise botocore120.exceptions.NoRegionError()
    ...
    def generate_presigned_url(self, request_dict, expires_in=3600,
        ...
        :param request_dict: The prepared request dictionary returned by
            ``botocore120.awsrequest.prepare_request_dict()``
...
class S3PostPresigner(object):
    ...
    def generate_presigned_post(self, request_dict, fields=None,
        ...
        policy['expiration'] = expire_date.strftime(botocore120.auth.ISO8601)
